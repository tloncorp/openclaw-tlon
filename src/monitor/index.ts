...
export async function monitorTlonProvider(opts: MonitorTlonOpts = {}): Promise<void> {
  ...
  const tlonConfig = cfg.channels?.tlon as { enabled?: boolean } | undefined;
  if (tlonConfig?.enabled === false) {return;}

  const logger = core.logging.getChildLogger({ module: "tlon-auto-reply" });
  const formatRuntimeMessage = (...args: any[]) => format(...args);
    ...
    await core.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
      ...
      dispatcherOptions: {
        ...
        deliver: async (payload: ReplyPayload) => {
          ...
          if (showSignature) {
            // Extended payload may include model info
            const extendedPayload = payload as any;
            const extendedRoute = route as any;
            const modelInfo =
              extendedPayload.metadata?.model ||
              extendedPayload.model ||
              extendedRoute.model ||
