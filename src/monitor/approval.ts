...
function shouldUsePendingApprovalsA2UI(activeApprovals: PendingApproval[]): boolean {
  return activeApprovals.length > 0 && activeApprovals.length <= MAX_PENDING_APPROVALS_A2UI;
}
...
export function buildPendingApprovalsA2UIBlob(
  ...
  const text = (id: string, value: string, variant?: A2UI.Text["variant"]): A2UI.Component =>
    variant
  ...
  for (const [index, approval] of active.entries()) {
    ...
    components.push(
      ...
      ...(fields.preview ? [text(`${prefix}Preview`, fields.preview, "caption")] : []),
      {
        id: `${prefix}Actions`,
        component: "Row",
        children: [`${prefix}Allow`, `${prefix}Reject`, `${prefix}Block`],
      },
      button(`${prefix}Allow`, "allowLabel", `/allow ${approval.id}`, "primary"),
      button(`${prefix}Reject`, "rejectLabel", `/reject ${approval.id}`),
      button(`${prefix}Block`, "blockLabel", `/ban ${approval.id}`, "borderless"),
    );
  }
...
export function buildPendingApprovalsResponse(
  ...
  return serialized ? { text, blob: serialized, mode: "ui" } : { text, mode: "text" };
}
