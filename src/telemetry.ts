...
function collectToolUsageSince(sessionKey: string, cursor: number): ToolUsageSummary {
  ...
  const calls =
    toolCallsBySession
      ...
      .map((call) => ({
        ...
        error: call.error,
        summaryKey: call.context?.kind === "tlonCommand" ? call.context.summaryKey : null,
      })) ?? [];
  const tlonContexts =
    toolCallsBySession
      .get(sessionKey)
  ...
  return {
    ...
    tlonChannelKinds: uniqueInOrder(
      tlonContexts.flatMap((call) => (call.channelKind ? [call.channelKind] : [])),
    ),
    tlonUpdateFields: uniqueInOrder(tlonContexts.flatMap((call) => call.updateFields ?? [])),
  };
}
