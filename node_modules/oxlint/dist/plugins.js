...
import { c as ObjectAssign, h as ObjectHasOwn, l as ObjectCreate, m as ObjectGetPrototypeOf, p as ObjectFreeze, u as ObjectDefineProperties, v as ObjectSetPrototypeOf } from "./globals.js";
const EMPTY_VISITOR = {};
function eslintCompatPlugin(plugin) {
	...
	for (let ruleName in rules) ObjectHasOwn(rules, ruleName) && convertRule(rules[ruleName]);
	return plugin;
}
function convertRule(rule) {
	...
	rule.create = (eslintContext) => (context === null && ({context, visitor, beforeHook} = createContextAndVisitor(rule)), ObjectDefineProperties(context, {
		id: { value: eslintContext.id },
		options: { value: eslintContext.options },
		report: { value: eslintContext.report }
	}), ObjectSetPrototypeOf(context, ObjectGetPrototypeOf(eslintContext)), beforeHook !== null && beforeHook() === !1 ? EMPTY_VISITOR : visitor);
}
