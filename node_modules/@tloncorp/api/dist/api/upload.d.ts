import { ImagePickerAsset } from '../domain/attachment';

/**
 * Minimal S3Client interface for upload functionality.
 * Compatible with @aws-sdk/client-s3's S3Client.
 */
...
/**
 * Zustand-compatible state setter type.
 */
...
export type MessageAttachments = ImagePickerAsset[];
...
interface UpdateStatus {
    ...
    (uploader: string, key: string, status: Omit<Status, 'error'>, msg?: string): void;
}
...
export type NativeUploader = (presignedUrl: string, file: RNFile, withPolicyHeader?: boolean, additionalHeaders?: Record<string, string>) => Promise<void>;
export interface FileStore {
    client: S3ClientLike | null;
    ...
    uploadFiles: (uploader: string, files: RNFile[] | null, config: StorageConfiguration, getImageSize: (url: string) => Promise<[number, number]>, nativeUploader?: NativeUploader) => Promise<void>;
    upload: (uploader: string, upload: Upload, config: StorageConfiguration, getImageSize: (url: string) => Promise<[number, number]>, compressor?: (file: RNFile) => Promise<RNFile>, nativeUploader?: NativeUploader) => Promise<void>;
    ...
    updateFile: (uploader: string, key: string, file: Partial<FileStoreFile>) => void;
